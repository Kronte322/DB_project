...
def _extract_credentials() -> Credentials:
    return Credentials(dbname=os.getenv("DBNAME", Credentials.dbname), host=os.getenv("DBHOST", Credentials.host),
                       port=os.getenv("DBPORT", Credentials.port), user=os.getenv("DBUSER", Credentials.user),
...
if __name__ == '__main__':
    ...
    cursor.execute("""INSERT INTO st.rank(rating_points, title) VALUES
    (0, 'Незнакомец'),
    (100, 'Бронза 1'),
    (200, 'Бронза 2'),
    (300, 'Бронза 3'),
    ...
    cursor.execute("""INSERT INTO st.game(start_dt, end_dt) VALUES
    ...
    df = pd.read_sql_query('SELECT * FROM st.user', con=sqlalchemy_conn())
    grouped = df.groupby('country_nm').size().reset_index(name='count')
    plt.bar(grouped['country_nm'], grouped['count'])
    ...
    df = pd.read_sql_query(
        """SELECT a.achivment_nm, COUNT(pa.user_id) AS count FROM st.achivment AS a JOIN 
        st.player_achivment AS pa ON a.achivment_id = pa.achivment_id GROUP BY a.achivment_id""",
        con=sqlalchemy_conn())
    plt.bar(df['achivment_nm'], df['count'])
    plt.xticks(rotation=90)
    plt.title('Achievement Popularity')
