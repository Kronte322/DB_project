...
def read_sql(filepath: str, flag=False) -> str:
...
def output_all_tables(schema: str) -> NoReturn:
    sql = f'SELECT * FROM information_schema.tables WHERE table_schema = \'{schema}\' AND table_type = \'BASE TABLE\';'
    sql = sql.replace("%", "%%")
    names_of_tables = list(execute_sql_to_df(conn=sqlalchemy_conn(), sql=sql)['table_name'])
    for table_name in names_of_tables:
        sql = f'SELECT * FROM {schema}.{table_name}'
        sql = sql.replace("%", "%%")
        print(f'{table_name} contains: ')
        print(execute_sql_to_df(conn=sqlalchemy_conn(), sql=sql))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tables",
                        default="",
                        dest="tables",
                        help="output all tables if SCHEMA")
